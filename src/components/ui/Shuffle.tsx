...
export const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { if (active) setFontsLoaded(true); }, 500);
    return () => {
      active = false;
      clearTimeout(timer);
    };
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        const metricsContext = isVertical && typeof document !== 'undefined' ? document.createElement('canvas').getContext('2d') : null;
